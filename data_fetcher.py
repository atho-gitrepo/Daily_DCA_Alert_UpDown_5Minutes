...
Version: 3.2.1 - FIXED: MongoDB integration, dependency management, improved error handling
"""
...
try:
    ...
    # Don't log warning here, will be handled in CacheManager

# Binance imports - with better error handling
...
class DataFetcher:
    ...
    def __init__(self, demo_mode: Optional[bool] = None):
        ...
        log_data_operation("INIT", "SUCCESS", 
                          {"demo_mode": self.demo_mode, "binance_available": BINANCE_AVAILABLE, "version": self.CACHE_VERSION}, 
                          emoji=EMOJI['SUCCESS'])
        data_logger.info(f"{EMOJI['SUCCESS']} DATA_INIT: DataFetcher v{self.CACHE_VERSION} initialized (demo: {self.demo_mode})")
    ...
    def _init_cache(self):
        ...
        data_logger.debug(f"{EMOJI['CACHE']} DATA_CACHE: Cache initialized")
    
    def _init_rate_limiter(self):
        self.rate_limiter = RateLimiter(
            max_requests_per_minute=config.performance.binance_rate_limit
        )
    ...
    def _init_binance_client(self):
        ...
        if self.demo_mode:
            data_logger.info(f"{EMOJI['DEMO']} DATA_INIT: Running in DEMO mode")
            return
            
        if not BINANCE_AVAILABLE:
            data_logger.warning(f"{EMOJI['WARNING']} DATA_INIT: Binance library not installed. Install with: pip install python-binance")
            return
        ...
        try:
            ...
            if not api_key or not api_secret:
                data_logger.warning(f"{EMOJI['WARNING']} DATA_INIT: Binance API credentials not configured. Running in demo mode.")
                self.demo_mode = True
                return
            ...
            data_logger.error(f"{EMOJI['ERROR']} DATA_INIT: Binance API error: {e.message if hasattr(e, 'message') else e}")
            self.metrics["api_errors"] += 1
            self.client = None
            self.demo_mode = True
            data_logger.warning(f"{EMOJI['DEMO']} DATA_INIT: Falling back to DEMO mode")
            
        except BinanceRequestException as e:
            data_logger.error(f"{EMOJI['ERROR']} DATA_INIT: Binance request error: {e}")
    ...
    def fetch_klines(self, symbol: str, interval: str, limit: int, 
        ...
        try:
            ...
            log_data_operation("FETCH", "FAILURE", 
                              ...
                              emoji=EMOJI['ERROR'])
            data_logger.error(f"{EMOJI['ERROR']} DATA_FETCH: Binance API error for {symbol}: {e.message if hasattr(e, 'message') else e}")
            return None
            ...
            log_data_operation("FETCH", "FAILURE", 
                              {"symbol": symbol, "error": str(e)}, 
                              emoji=EMOJI['ERROR'])
            data_logger.error(f"{EMOJI['ERROR']} DATA_FETCH: Connection error for {symbol}: {e}")
            return None
    ...
    def _fetch_live_klines(self, symbol: str, interval: str, limit: int) -> Optional[pd.DataFrame]:
        ...
        max_retries = getattr(config.performance, 'max_retries', 3)
        retry_delay = getattr(config.performance, 'retry_delay_seconds', 5)
        ...
        for attempt in range(max_retries):
            try:
                ...
                data_logger.error(f"{EMOJI['ERROR']} DATA_FETCH: Unexpected error for {symbol}: {e}")
                if attempt < max_retries - 1:
    ...
    def fetch_multiple_timeframes(self, symbol: str, intervals: List[str], limit: int,
        ...
        Args:
            symbol: Trading pair symbol
            intervals: List of intervals to fetch (e.g., ['5m', '15m', '1h'])
            limit: Number of candles per timeframe
            heikin_ashi: Whether to calculate Heikin Ashi
        """
        results = {}
